...
<?php 
include "conn.php";
$sql = mysqli_query($conn, "SELECT MIN(watertemp) as watertemp FROM `waterreporttbl` WHERE DATE LIKE CURRENT_DATE");
...
<div>
<?php 
$sql1 = mysqli_query($conn, "SELECT MIN(watertemp) as watertemp FROM `waterreporttbl` WHERE MONTH(DATE) LIKE MONTH(CURRENT_DATE)");
